function Tetris(options) {
  ...
  let width = (+options.width < 10) ? 10 : ~~(+options.width);
  let height = (+options.height < 20) ? 21 : ~~(+options.height + 1);
  ...
  function generateComponent() {
    ...
    function generateGlassUI() {
      let divGameField = document.createElement('div');
  ...
  Tetramino.prototype.rotate = function() {
    let temp = new Array(this.form[0].length);
  ...
  function drawNextTetramino() {
    ...
    for (let i = 0; i < nextTetramino.form.length; i++) {
      for (let j = 0; j < nextTetramino.form[i].length; j++) {
  ...
  function moveTetramino() {
    controlTimeoutHandle = setTimeout(function() {
      ...
      if (currentTetramino.prevCoords) {
        for (let i = 0; i < currentTetramino.form.length; i++) {
          for (let j = 0; j < currentTetramino.form[i].length; j++) {
  ...
  function checkBlocksUnderTetramino() {
    ...
    for (let i = currentTetramino.form.length - 1; i >= 0; i--) {
      for (let j = currentTetramino.form[i].length - 1; j >= 0; j--) {
  ...
  function checkSideCoords(coords, side) {
    ...
    let side = (side === -1) ? -1 : currentTetramino.form[0].length;
